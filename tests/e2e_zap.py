...
target_url = os.getenv('TARGET_URL', 'http://localhost:5050')
...
try:
    login = requests.post(f"{target_url}/login",
...
try:
    ...
    get_cust_id = requests.get(
        f"{target_url}/get/2", proxies=proxies, headers=auth_header, verify=False, timeout=30)
...
try:
    ...
    fetch_customer_post = requests.post(
        f"{target_url}/fetch/customer", json=post, proxies=proxies, headers=auth_header, verify=False, timeout=30)
...
try:
    ...
    search_customer_username = requests.post(
        f"{target_url}/search", json=search, proxies=proxies, headers=auth_header, verify=False, timeout=30)
...
try:
    zap = ZAP(proxies={'http': f'http://{zap_host}:{zap_port}', 'https': f'http://{zap_host}:{zap_port}'})
...
urls_to_visit = [
    ...
    f"{target_url}/get/1",
    f"{target_url}/search",
    f"{target_url}/fetch/customer",
    f"{target_url}/register/user",
    f"{target_url}/register/customer",
    f"{target_url}/xxe",
    f"{target_url}/yaml"
]
